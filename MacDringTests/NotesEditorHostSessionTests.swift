...
final class NotesEditorHostSessionTests: XCTestCase {
    ...
    func testNothingIsSentBeforeReady() {
        ...
        session.markReady()
        XCTAssertEqual(session.nextMessage(), .initialize(markdown: "# a", theme: "light", revision: 1))
    }
    ...
    func testEditorChangeIsNotEchoedBack() {
        ...
        session.recordEditorChange("# a typed")
    ...
    func testSwitchingDocumentInitializesWithANewRevision() {
        ...
        XCTAssertEqual(session.nextMessage(), .initialize(markdown: "# b", theme: "light", revision: 2))
    }
    ...
    func testRevisionsAreStrictlyIncreasingAcrossMessages() {
        ...
        session.setDesired(documentID: tabA, markdown: "# a", theme: .light)
        var revisions: [Int] = []
        if case let .initialize(_, _, revision)? = session.nextMessage() { revisions.append(revision) }
