...
- **Favicons.** For web links the user has added, Top Drawer fetches `https://<host>/favicon.ico` from that link's own site to show its icon in the drawer. The app itself keeps the decoded icons only in memory for the session; the system's standard HTTP cache may additionally retain the responses on disk under `~/Library/Caches/com.macdring.MacDring`, as with any macOS network request.
...
Removing Top Drawer, its Application Support folder, and its preferences domain (`defaults delete com.macdring.MacDring` — user defaults also hold the Recents history) removes all of its data; the system HTTP cache lives under `~/Library/Caches/com.macdring.MacDring`. Questions or security reports can be submitted through the project's GitHub repository.
