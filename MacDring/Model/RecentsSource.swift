...
/// Where a `.recents` tab draws its history from. The classic source is Top Drawer's
...
enum RecentsSource: String, Codable, CaseIterable, Identifiable, Equatable {
    /// Only targets opened from Top Drawer itself (the original behavior, and the default).
    case macDring
    ...
    var displayName: String {
        switch self {
        case .macDring: return "Top Drawer"
        case .system: return "System"
        case .both: return "Both"
        }
    }

    /// Whether this source includes Top Drawer's own launch history (read synchronously).
