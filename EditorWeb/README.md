...
- Timings and memory, measured by `npm run smoke` in `EditorWeb CI` (headless
  Chromium, `ubuntu-latest`, assets served over loopback): page load 114 ms,
  mount after `initialize` 83 ms, warm reopen (reload plus `initialize`) 131 ms,
  heap 9.5 MiB after the first mount and unchanged after ten document swaps, so
  repeated open/close does not leak. These are browser-side numbers from a
  different engine and a faster machine class than the drawers' target, so treat
  them as an order of magnitude and a leak check, not as the WKWebView budget.
  The plan's in-app cold/warm budget still needs measuring on a real session.
