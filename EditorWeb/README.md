...
- Compatibility caveat: Crepe's stylesheets use `color-mix()` (Safari 16.2+).
  macOS 13.0–13.2 ship WebKit 16.1, so on those systems some surface colors
  fall back to their unset value. Phase 2 must either raise the notes editor's
  minimum to 13.3 or ship `color-mix` fallbacks. `:has()` is fine (Safari 15.4+).
